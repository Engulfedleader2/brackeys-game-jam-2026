...
func _init(c_instance: CardInstance, c_owner_id: int, c_face_down: bool, c_play_index: int, c_declared: int) -> void:
	instance = c_instance
	face_down = c_face_down
	owner_id = c_owner_id
	play_index = c_play_index
	declared_value = c_declared
...
func effective_value() -> int:
	var value := declared_value if face_down else base_value()
	return -value if instance.negated else value
